...
# Script: figure_4_6_violin_plots_with_ferritin_index_landscape.R
...
# - ferritin index: allow ferritin_index or ferrtin_index (typo variant)
...
# - user has renamed 'iron' to TIBC (so look for tibc explicitly)
...
plot_one <- function(bm, show_legend = FALSE) {
  ...
  # avoid log10 on non-positive values
  ...
  # Ferritin index / transferrin / TIBC: no universal clinical cut-offs in pregnancy,
  # so leave without threshold by default.
  ...
  subtitle_txt <- case_when(
    ...
    bm == "Transferrin"  ~ "No threshold",
    bm == "TIBC"         ~ "No threshold",
    TRUE ~ NA_character_
  )
...
# ================== Save (A4 landscape) ==================
ggsave("biomarkers_A4_landscape_3x4_with_FI_Tf_TIBC.pdf",
       combined, width = 11.69, height = 8.27, dpi = 300)
ggsave("biomarkers_A4_landscape_3x4_with_FI_Tf_TIBC.png",
       combined, width = 11.69, height = 8.27, dpi = 300)
